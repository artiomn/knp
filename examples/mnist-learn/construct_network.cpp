...
AnnotatedNetwork create_example_network(int num_compound_networks)
{
    ...
    for (int i = 0; i < num_compound_networks; ++i)
    {
        ...
        ResourceDeltaProjection input_projection{
            knp::core::UID{false}, population_uids[0], make_dense_generator(28 * 28, afferent_synapse), 28 * 28 * 150};
        ...
        ResourceDeltaProjection projection_1{
            population_uids[0], population_uids[1], make_1_to_1_generator(default_synapse), 150};
        result.network_.add_projection(projection_1);
        if (result.data_.inference_population_uids.find(population_uids[0]) !=
                result.data_.inference_population_uids.end() &&
            result.data_.inference_population_uids.find(population_uids[1]) !=
                result.data_.inference_population_uids.end())
        {
        ...
        DeltaProjection wta_projection{
            population_uids[1], population_uids[1], make_all_to_all_sections_generator(15, default_wta_synapse),
            pop_data[1].pd.size_ * pop_data[1].pd.size_};
        result.network_.add_projection(wta_projection);
        if (result.data_.inference_population_uids.find(population_uids[1]) !=
            result.data_.inference_population_uids.end())
        {
        ...
        DeltaProjection projection_2{
            population_uids[1], population_uids[2],
            make_aligned_generator(pop_data[1].pd.size_, pop_data[2].pd.size_, default_activating_synapse),
            pop_data[1].pd.size_};
        result.network_.add_projection(projection_2);
        if (result.data_.inference_population_uids.find(population_uids[1]) !=
                result.data_.inference_population_uids.end() &&
            result.data_.inference_population_uids.find(population_uids[2]) !=
                result.data_.inference_population_uids.end())
        {
        ...
        DeltaProjection projection_3{
            population_uids[2], population_uids[0],
            make_aligned_generator(pop_data[2].pd.size_, pop_data[0].pd.size_, default_dopamine_synapse),
            pop_data[0].pd.size_};
        ...
        DeltaProjection projection_4{
            population_uids[1], population_uids[3],
            make_aligned_generator(pop_data[1].pd.size_, pop_data[3].pd.size_, default_synapse), pop_data[1].pd.size_};
        result.network_.add_projection(projection_4);
        if (result.data_.inference_population_uids.find(population_uids[1]) !=
                result.data_.inference_population_uids.end() &&
            result.data_.inference_population_uids.find(population_uids[3]) !=
                result.data_.inference_population_uids.end())
        {
        ...
        DeltaProjection projection_5{
            population_uids[3], population_uids[4],
            make_aligned_generator(pop_data[3].pd.size_, pop_data[4].pd.size_, default_blocking_synapse), 10};
        result.network_.add_projection(projection_5);
        if (result.data_.inference_population_uids.find(population_uids[3]) !=
                result.data_.inference_population_uids.end() &&
            result.data_.inference_population_uids.find(population_uids[4]) !=
                result.data_.inference_population_uids.end())
        {
        ...
        DeltaProjection projection_6{
            knp::core::UID{false}, population_uids[2],
            make_aligned_generator(10, pop_data[2].pd.size_, default_synapse), pop_data[2].pd.size_};
        ...
        DeltaProjection projection_7{
            knp::core::UID{false}, population_uids[4], make_aligned_generator(10, pop_data[4].pd.size_, slow_synapse),
            pop_data[4].pd.size_};
        ...
        DeltaProjection projection_8{
            knp::core::UID{false}, population_uids[4], make_exclusive_generator(10, inhibitory_synapse),
            10 * (pop_data[4].pd.size_ - 1)};
        ...
        DeltaProjection projection_9{
            population_uids[4], population_uids[0],
            make_aligned_generator(pop_data[4].pd.size_, pop_data[0].pd.size_, weak_excitatory_synapse),
            pop_data[0].pd.size_};
